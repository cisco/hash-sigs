/*
 * This will test out the test vector found in the draft
 *
 * This might seem a trivial test, however it's the only place where to
 * validate ourselves against another implementation
 */

#include "hss.h"
#include "test_hss.h"

bool test_testvector_1(void) {
    /* This is test case 1 */
    static const unsigned char public_key[] = {
        0x00,0x00,0x00,0x02,0x00,0x00,0x00,0x05,
        0x00,0x00,0x00,0x04,0x61,0xa5,0xd5,0x7d,
        0x37,0xf5,0xe4,0x6b,0xfb,0x75,0x20,0x80,
        0x6b,0x07,0xa1,0xb8,0x50,0x65,0x0e,0x3b,
        0x31,0xfe,0x4a,0x77,0x3e,0xa2,0x9a,0x07,
        0xf0,0x9c,0xf2,0xea,0x30,0xe5,0x79,0xf0,
        0xdf,0x58,0xef,0x8e,0x29,0x8d,0xa0,0x43,
        0x4c,0xb2,0xb8,0x78,
    };

    static const unsigned char message[] = {
        ,
    };

    static const unsigned char signature[] = {
        ,
    };

    bool verified = hss_validate_signature(
                          public_key,
                          message, sizeof message,
                          signature, sizeof signature, 0);

    return verified;
}

bool test_testvector_2(void) {
    /* This is test case 2 */
    static const unsigned char public_key[] = {
        0x00,0x00,0x00,0x02,0x00,0x00,0x00,0x06,
        0x00,0x00,0x00,0x03,0xd0,0x8f,0xab,0xd4,
        0xa2,0x09,0x1f,0xf0,0xa8,0xcb,0x4e,0xd8,
        0x34,0xe7,0x45,0x34,0x32,0xa5,0x88,0x85,
        0xcd,0x9b,0xa0,0x43,0x12,0x35,0x46,0x6b,
        0xff,0x96,0x51,0xc6,0xc9,0x21,0x24,0x40,
        0x4d,0x45,0xfa,0x53,0xcf,0x16,0x1c,0x28,
        0xf1,0xad,0x5a,0x8e,
    };
    static const unsigned char message[] = {
        ,
    };

    static const unsigned char signature[] = {
        ,
    };

    bool verified = hss_validate_signature(
                          public_key,
                          message, sizeof message,
                          signature, sizeof signature, 0);

    return verified;
}

bool test_testvector_3(void) {
    /* This is test case 3 */
    /* This is the SHA256/192 test from draft-fluhrer-lms-more-parm_sets */
    static const unsigned char public_key[] = {
        0x00,0x00,0x00,0x01,0xe0,0x00,0x00,0x01,
        0xe0,0x00,0x00,0x04,
        0x20,0x21,0x22,0x23,0x24,0x25,0x26,0x27,
        0x28,0x29,0x2a,0x2b,0x2c,0x2d,0x2e,0x2f,
        0x2c,0x57,0x14,0x50,0xae,0xd9,0x9c,0xfb,
        0x4f,0x4a,0xc2,0x85,0xda,0x14,0x88,0x27,
        0x96,0x61,0x83,0x14,0x50,0x8b,0x12,0xd2,
    };
    static const unsigned char message[] = {
        0x54,0x65,0x73,0x74,0x20,0x6d,0x65,0x73,
        0x73,0x61,0x67,0x65,0x20,0x66,0x6f,0x72,
        0x20,0x53,0x48,0x41,0x32,0x35,0x36,0x2d,
        0x31,0x39,0x32,0x0a,
    };
    static const unsigned char signature[] = {
        ,
    };

    bool verified = hss_validate_signature(
                          public_key,
                          message, sizeof message,
                          signature, sizeof signature, 0);

    return verified;
}

bool test_testvector_4(void) {
    /* This is test case 4 */
    /* This is the SHAKE256/192 test from draft-fluhrer-lms-more-parm_sets */
    static const unsigned char public_key[] = {
        0x00,0x00,0x00,0x01,0xe0,0x00,0x00,0x0b,
        0xe0,0x00,0x00,0x0c,
        0x50,0x51,0x52,0x53,0x54,0x55,0x56,0x57,
        0x58,0x59,0x5a,0x5b,0x5c,0x5d,0x5e,0x5f,
        0xdb,0x54,0xa4,0x50,0x99,0x01,0x05,0x1c,
        0x01,0xe2,0x6d,0x99,0x90,0xe5,0x50,0x34,
        0x79,0x86,0xda,0x87,0x92,0x4f,0xf0,0xb1,
    };
    static const unsigned char message[] = {
        0x54,0x65,0x73,0x74,0x20,0x6d,0x65,0x73,
        0x73,0x61,0x67,0x65,0x20,0x66,0x6f,0x72,
        0x20,0x53,0x48,0x41,0x4b,0x45,0x32,0x35,
        0x36,0x2d,0x31,0x39,0x32,0x0a,
    };
    static const unsigned char signature[] = {
        ,
    };

    bool verified = hss_validate_signature(
                          public_key,
                          message, sizeof message,
                          signature, sizeof signature, 0);

    return verified;
}

bool test_testvector_5(void) {
    /* This is test case 5 */
    /* This is the SHAKE256/256 test from draft-fluhrer-lms-more-parm_sets */
    static const unsigned char public_key[] = {
        0x00,0x00,0x00,0x01,0xe0,0x00,0x00,0x06,
        0xe0,0x00,0x00,0x08,
        0x80,0x81,0x82,0x83,0x84,0x85,0x86,0x87,
        0x88,0x89,0x8a,0x8b,0x8c,0x8d,0x8e,0x8f,
        0x9b,0xb7,0xfa,0xee,0x41,0x1c,0xae,0x80,
        0x6c,0x16,0xa4,0x66,0xc3,0x19,0x1a,0x8b,
        0x65,0xd0,0xac,0x31,0x93,0x2b,0xbf,0x0c,
        0x2d,0x07,0xc7,0xa4,0xa3,0x63,0x79,0xfe,
    };
    static const unsigned char message[] = {
        0x54,0x65,0x73,0x74,0x20,0x6d,0x65,0x73,
        0x61,0x67,0x65,0x20,0x66,0x6f,0x72,0x20,
        0x53,0x48,0x41,0x4b,0x45,0x32,0x35,0x36,
        0x2d,0x32,0x35,0x36,0x0a,
    };
    static const unsigned char signature[] = {
        ,
    };

    bool verified = hss_validate_signature(
                          public_key,
                          message, sizeof message,
                          signature, sizeof signature, 0);

    return verified;
}

bool test_testvector(bool fast_flag, bool quiet_flag) {
    return test_testvector_1() &&
           test_testvector_2() &&
           test_testvector_3() &&
	   test_testvector_4() &&
	   test_testvector_5();
}
