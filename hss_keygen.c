...
bool (*generate_random)(void *output, size_t length), unsigned levels,
		const param_set_t *lm_type, const param_set_t *lm_ots_type,
		unsigned char *sk, void *context, unsigned char *public_key,
		size_t len_public_key, unsigned char *aux_data, size_t len_aux_data,
	...
	unsigned char private_key[PRIVATE_KEY_LEN] = { 0 };
	...
	if (!hss_generate_root_seed_I_value(seed, I,
...
#if ALLOW_VERBOSE
	printf("\nhss_generate_root_seed_I_value\n");
		for (unsigned long long j = 0; j < PRIVATE_KEY_LEN; j++) {
			printf("%x,", private_key[j]);
		}
		printf("\n");
...
#define MAX_INCREMENT 20000
	...
	for (j = 0; j < level_nodes;) {
		...
		hss_thread_issue_work(col, hss_gen_intermediate_tree, &details,
				sizeof details);
	...
	for (r = level_nodes, leaf_node = 0; leaf_node < level_nodes;
			r++, leaf_node++) {
		...
		for (sp = 1;; sp++, cur_lev--, q >>= 1) {
			...
			if (sp > 1) {
				hss_save_aux_data(expanded_aux_data, cur_lev, size_hash, q,
						current_buf);
			}
			...
			hss_combine_internal_nodes(current_buf,
					&stack[(sp - 1) * size_hash], current_buf, h, I, size_hash,
					r >> sp);
		}
	}
	...
	public_key += I_LEN;
	len_public_key -= I_LEN;
	memcpy(public_key, root_hash, size_hash);
	public_key += size_hash;
	len_public_key -= size_hash;

	memcpy(sk, &private_key, sizeof(unsigned char) * PRIVATE_KEY_LEN);
 ...
 */
size_t hss_get_private_key_len(unsigned levels, const param_set_t *lm_type,
